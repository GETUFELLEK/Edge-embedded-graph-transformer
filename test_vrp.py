...
def evaliuate(valid_loder,batch_size,n_node):
    ...
    # filepath = '/home/dl-box/RoutingwithGTransformer/vrp-{}-gf/ppo21results2021-11-13 10:33:17.670020/72'
    # =6.18 result with embedding of 512 qnd bachsize 128
    # =6.2111 BRAVO node embedding is 512 and bachsize is 128'
    # filepath = '/home/dl-box/RoutingwithGTransformer/vrp-{}-gf/ppo21results2021-12-01 20:10:58.611987/70'
...
def test(n_node, real_data=False):
    datas = []
    ...
    if real_data:
        if n_node==21 or n_node==51 or n_node==101:
           node_ = np.loadtxt('/home/dl-box/RoutingwithGTransformer/test_data/vrp20_test_data.csv'.format(n_node-1), dtype=np.float, delimiter=',')
           demand_ = np.loadtxt('/home/dl-box/RoutingwithGTransformer/test_data/vrp20_demand.csv'.format(n_node-1), dtype=np.float, delimiter=',')
           capcity_ = np.loadtxt('/home/dl-box/RoutingwithGTransformer/test_data/vrp20_capcity.csv'.format(n_node-1), dtype=np.float, delimiter=',')
        ...
        dl = pickle.load(open('hibikino_vrp_dl_' + str(n_node) + '.pkl', 'rb'))
