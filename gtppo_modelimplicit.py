...
import time
from multiheadforimplicitattention import MultiHeadAttentionLayer
...
class GraphAttentionEncoder(nn.Module):
    def __init__(self,input_node_dim, hidden_node_dim,input_edge_dim,hidden_edge_dim,layers=3,n_heads=4,dropout=0.0,batch_norm=True, residual=True,feed_forward_hidden=512
    ):
        ...
        # for both inputs we need to map them to embedding space as in most cases and passing it to the attention layer
        # self.hidden_node_dim = hidden_node_dim
        self.init_embed_node = nn.Linear(input_node_dim, hidden_node_dim)
#         if node_dim is not None else None
        self.init_embed_edge = nn.Linear(input_edge_dim, hidden_edge_dim)

        self.layers = nn.ModuleList([MultiHeadAttentionLayer(hidden_node_dim,hidden_edge_dim,n_heads,dropout,batch_norm,residual,feed_forward_hidden)
            for _ in range(layers)])
        self.layers.append(MultiHeadAttentionLayer(hidden_node_dim,hidden_edge_dim,n_heads,dropout,batch_norm,residual,feed_forward_hidden))
        # self.layers = MultiHeadAttentionLayer((
        #                                       input_node_dim, hidden_node_dim, input_edge_dim, hidden_edge_dim, n_heads,
        #                                       dropout, batch_norm, residual, feed_forward_hidden)
        #                                       for _ in range(layers))

    def forward(self,data,mask=None):
    # def forward(self, x,e,edge_index mask=None):
        ...
        # concatenate to get the features of nodes combined as in GANN
        '''we can optionally consider to have batch normalization for initial embeddings of node and edge '''
        x = torch.cat([data.x, data.demand], -1)
        h = self.init_embed_node(x)
        h = h.view(batch_size,-1,self.hidden_node_dim)

        # h = self.init_embed_node(x.view(-1, x.size(-1))).view(*x.size()[:2], -1)
        e= self.init_embed_edge(data.edge_attr)
        # e=e.reshape(batch_size,n_nodes,self.hidden_node_dim,-1)
        # h_out,e_out = self.layers(h,e,data.edge_index)
        for conv in self.layers:
            h_out= conv(h, e)

        # h_out = h_out.mean,

        h_out = h_out.reshape((batch_size,-1, self.hidden_node_dim))
        return h_out
    #  x = torch.concat(h.mean, e.mean)
    # def forward(self, x, edge_attr, edge_index, mask=None):
    #         # def forward(self, x,e,edge_index mask=None):
    #
    #         assert mask is None, "TODO mask not yet supported!"
    #         # batch_size = data.num_graphs
    ...
    #         # concatenate to get the features of nodes combined as in GANN
    #         '''we can optionally consider to have batch normalization for initial embeddings of node and edge '''
    #         # x = torch.cat([x.data, data.demand], -1)
    #         h = self.init_embed_node(x)
    #         e = self.init_embed_edge(edge_attr)
    #
    #         h = self.laysers(h, e, edge_index)
...
#          x = x.reshape((batch_size, -1, self.hidden_node_dim)) Am adding this reshaping from GANN code
...
class Model(nn.Module):
    def __init__(self,input_node_dim, hidden_node_dim, input_edge_dim, hidden_edge_dim, laysers):
        super(Model, self).__init__()
        self.encoder = GraphAttentionEncoder(input_node_dim, hidden_node_dim, input_edge_dim, hidden_edge_dim,
                                             laysers)
...
class Actor_critic(nn.Module):
    def __init__(self, input_node_dim, hidden_node_dim, input_edge_dim, hidden_edge_dim,laysers):
        ...
        self.actor = Model(input_node_dim, hidden_node_dim, input_edge_dim, hidden_edge_dim, laysers)
...
class Agentppo:
    def __init__(self, steps, greedy, lr, input_node_dim, hidden_node_dim, input_edge_dim, hidden_edge_dim, epoch=1,
                 batch_size=32, laysers=3, entropy_value=0.2, eps_clip=0.2):
        self.policy = Actor_critic(input_node_dim,hidden_node_dim,input_edge_dim,hidden_edge_dim,laysers)
